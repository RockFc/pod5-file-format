...
先读第 1.1 节的体积结论和第 1.2 节的实测压缩比，第 2.1 节一次 Run 如何拆成多个文件，再读第 5 节的 VBZ 逐步算法与例子。
...
上面 4:1 是按 VBZ 信号本体的乐观粗算。落到磁盘上的 `.pod5` 还含表和容器，真实 ADC 也没有教科书那么平滑。第 1.2 节用两套真实数据测到的**整文件**比（原始 `int16` / `.pod5`）大约是 **2.3～2.4:1**，空间节省约 **57%**。按这个口径外推一次 Run：未压缩 3.2 TB 的信号大约落到 **1.3～1.4 TB** 的 POD5，而不是 0.8 TB。数量级仍然成立，只是别把 4:1 当成盘上一定能看到的数字。
...
一句话：**一次 48 h、5000 Hz、3200 万 read 的 Run，未压缩信号是 TB 级，Reads/Run Info 合计大约数 GB；POD5 能把测序数据存得这么紧，核心就是对信号做 VBZ。** 算法逐步说明见第 5 节。实测整文件大约能压到原始 `int16` 的 **四成出头**（约 2.3～2.4:1），见下一节。

### 1.2 实测：两套真实 POD5 的整文件压缩比

方法：`[mytest4]` 打开每个 `.pod5`，把全部 read 的 ADC 解成裸小端 `int16` 并拼成同名 `.dat`。压缩比定义为：
...
| 数据集 | 文件数 | Reads | 原始 int16 | `.pod5` | 整文件比 | 空间节省 | 平均 \(T_{\text{avg}}\) |
|--------|--------|-------|------------|---------|----------|----------|-------------------------|
| `AMtb_1__202402`（单 flow cell `FAY22732`，barcode81） | 47 | 186 332 | **17.289 GB** | **7.376 GB** | **2.344** | **57.3%** | **9.3 s** |
| `Klebsiella_pneumoniae_KPC2`（4 个 barcode/run 混在同一目录） | 14 | 49 146 | **10.768 GB** | **4.537 GB** | **2.374** | **57.9%** | **21.9 s** |

单文件比非常齐，几乎不随文件体积涨落：

| 数据集 | 单文件比 min～max | 单文件比中位数 | `.pod5` 体积 | 每文件 reads |
|--------|-------------------|----------------|--------------|--------------|
| AMtb | 2.325～2.379 | 2.343 | 87～179 MB（末批 87 MB，其余约 140～179 MB） | 除末份 2332 外均为 **4000** |
| Klebsiella | 2.340～2.397 | 2.380 | 47～501 MB | 满批 **4000**，各 run 末批 624～3422 |

读下来有几条很硬的结论：

1. **真实整文件大约是 2.3～2.4:1，不是 4:1。** 两套菌种、read 长短差一倍（9 s vs 22 s），压缩比几乎一样，说明对这类纳米孔 ADC，VBZ+容器的落盘收益相当稳定，大约能把原始 `int16` 削掉 **57%**。
2. **体积仍然几乎全是信号。** AMtb 平均每条 read 约 46 k 个采样点 → 约 93 KB 裸 `int16`，对应 `.pod5` 约 40 KB/read；元数据仍是每行几百字节，占比可以忽略。
3. **AMtb 的 9.3 s 平均时长** 正好落在第 1.1 节「10 秒、很常见」那一档。把它的 2.34:1 套回 3200 万 read、10 秒平均：未压缩约 3.2 TB，POD5 约 **1.37 TB**。
4. **切文件看 reads 数，不看「像不像一小时」。** 见第 2.1 节：这两套数据几乎每个满批文件都是 4000 条 read。
...
|----------------------------------------|--------|------------|------------|
| `FAV69578_pass_barcode57_...` | 4 | 4000×3 | 1862 |
| `FAX39616_pass_barcode14_...` | 2 | 4000×1 | 3422 |
| `FAX39617_pass_barcode63_...` | 4 | 4000×3 | 624 |
| `FAX76295_pass_barcode23_...` | 4 | 4000×3 | 3238 |

同样是满批 4000、末批收尾。单文件最大约 **501 MB**，因为平均 read 更长（约 22 s），同样 4000 条就要占更多磁盘。

所以：一个 `.pod5` 仍是 **这一批里已经结束的多通道 reads**，不是按孔各一个文件，也不是整次 Run 的连续波形。判断怎么切，优先看 **每文件 read 数是不是常数**（本数据是 4000），再看 `batch_number` 和时间窗。
...
`[mytest2]` 用 `iota` 生成 20 个点：`-10, -9, ..., 9`。相邻差恒为 +1，zigzag 后几乎全是 `2`，svb16 的 data 区会非常规整，再叠加 zstd，压缩比会明显高于「按 int16 原样存储」。`[mytest3]` 对真实 `reads_all.dat` 做同一套预压缩，打印的 ratio / space savings 就是「原始 2N 字节 vs VBZ blob」的对比（还没算 Arrow/容器开销）。`[mytest4]` 则从真实 `.pod5` 解回全部 `int16`，用整文件字节数做分母；第 1.2 节两套数据都落在 **2.3～2.4:1**，说明信号占绝对大头时，整文件比会略低于纯 VBZ blob 比（表和容器也占一点，真实 ADC 也没有 iota 序列那么好压）。
